...
class Grass:
    ...
    async def start(self):
        ...
        while True:
            try:
                if retry >= max_retry:
                    self.log(f"{yellow}Max retries reached, skipping the proxy!")
                ...
                async with self.ses.ws_connect(
                    ...
                    self.log(f"{green}Successfully connected {white}to server!")
                    retry = 1
                    while True:
                        ping_data = {
...
async def main():
    ...
    {red}  This script was originally open-sourced by Snow God of War@Hy78516012, modified by Ostinz for free use.
    {white}GitHub    : {green}https://github.com/OstinZ
    {white}Twitter   : {green}https://x.com/sc0larz
    {green}Get some grass!
          """
    )
    
    # Checking and prompting for token.txt
    ...
    # Checking and prompting for userid.txt
